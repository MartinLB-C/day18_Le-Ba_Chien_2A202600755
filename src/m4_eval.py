...
def evaluate_ragas(questions: list[str], answers: list[str],
    ...
    if not ALIBABA_API_KEY:
        print("  ⚠️  Missing ALIBABA_API_KEY for RAGAS evaluation")
    ...
    try:
        ...
        from config import EMBEDDING_MODEL, LLM_BASE_URL, LLM_MODEL_NAME

        alibaba_llm = ChatOpenAI(
            api_key=ALIBABA_API_KEY,
            base_url=LLM_BASE_URL,
            model=LLM_MODEL_NAME
        )
        alibaba_embeddings = HuggingFaceEmbeddings(model_name=EMBEDDING_MODEL)
        ...
        result = evaluate(dataset, metrics=[faithfulness, answer_relevancy,
                          ...
                          llm=alibaba_llm, embeddings=alibaba_embeddings)
        df = result.to_pandas()
