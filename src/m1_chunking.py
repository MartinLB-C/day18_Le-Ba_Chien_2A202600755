...
def chunk_semantic(text: str, threshold: float = SEMANTIC_THRESHOLD,
    ...
    for i in range(1, len(sentences)):
        ...
        if sim < threshold:
            chunks.append(Chunk(text=" ".join(current_chunk), metadata={**metadata, "strategy": "semantic", "chunk_index": len(chunks)}))
...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    for p in parents:
        ...
        # chunk child text
        child_paras = [cp.strip() for cp in p.text.split("\n\n") if cp.strip()]
        current_child_text = ""
...
def chunk_structure_aware(text: str, metadata: dict | None = None) -> list[Chunk]:
    ...
    for sec in sections:
        if re.match(r'^#{1,3}\s+', sec):
            if current_content.strip():
                chunks.append(Chunk(
                    text=f"{current_header}\n{current_content}".strip() if current_header != "Intro" else current_content.strip(),
    ...
    if current_content.strip() or current_header != "Intro":
        chunks.append(Chunk(
            text=f"{current_header}\n{current_content}".strip() if current_header != "Intro" else current_content.strip(),
            metadata={**metadata, "section": current_header, "strategy": "structure", "chunk_index": len(chunks)}
        ))
